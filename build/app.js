...
(function () {
  ...
  var hoveredId = null;   // site id currently enlarged from a list hover
  ...
  function renderMarkers(sites) {
    ...
    markers = {};
    hoveredId = null; // the old marker object is gone; stop tracking it
    ...
    display.forEach(function (item) {
      ...
      var baseRadius = selected ? 11 : approx ? 10 : 6;
      var baseWeight = selected ? 3 : approx ? 2 : 1.5;
      ...
      // Stashed so hoverSite()/clearHover() can restore the exact size
      // without recomputing the selected/approx/band logic above.
      marker._baseRadius = baseRadius;
      marker._baseWeight = baseWeight;
  ...
  /**
   * Enlarge a site's marker when its row in the results list is hovered.
   * No-ops if the site is currently folded into a cluster bubble rather than
   * drawn as its own marker — there is nothing on screen to point at.
   */
  function hoverSite(id) {
    if (hoveredId === id) return;
    clearHover();
    var marker = markers[id];
    if (!marker) return;
    hoveredId = id;
    marker.setStyle({ radius: marker._baseRadius + 5, weight: marker._baseWeight + 1.5 });
    marker.bringToFront();
  }

  function clearHover() {
    if (!hoveredId) return;
    var marker = markers[hoveredId];
    hoveredId = null;
    if (marker) marker.setStyle({ radius: marker._baseRadius, weight: marker._baseWeight });
  }
  ...
  function initControls() {
    ...
    // mouseenter/mouseleave don't bubble, so delegate via mouseover/mouseout
    // (like the click handler above) rather than re-binding per row on every
    // re-render. Guard mouseout against moving between children of the same
    // .result — only clear once the pointer actually leaves the row.
    el['results'].addEventListener('mouseover', function (e) {
      var btn = e.target.closest ? e.target.closest('.result') : null;
      if (btn && btn.dataset.id) hoverSite(btn.dataset.id);
    });
    el['results'].addEventListener('mouseout', function (e) {
      var btn = e.target.closest ? e.target.closest('.result') : null;
      if (btn && (!e.relatedTarget || !btn.contains(e.relatedTarget))) clearHover();
    });
